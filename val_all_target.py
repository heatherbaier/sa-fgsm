...
#     parser.add_argument('--labels_path', required = False, default = "/sciclone/geograd/Heather/c1/data/clean/c2_ys.json")
    ...
    parser.add_argument('--coords_path', required = False, default = "/sciclone/geograd/heather_data/ti/data/central_asia_coords_target.json")
...
#     if args.start_epoch == 0:
    if not os.path.exists(f"{folder_name}/{args.target_name}_results/"):
        os.mkdir(f"{folder_name}/{args.target_name}_results/")
        
    epoch = args.start_epoch

    # for epoch in reversed(range(args.start_epoch, 71)):
...
#         epoch = 88
    ...
    #                 
        ...
        if count % 5000 == 0:
            ...
            df.to_csv(f"{folder_name}/{args.target_name}_results/epoch{epoch}_target_preds.csv", index = False)                
